...
<main class="max-w-3xl mx-auto px-6 py-20 text-center">
    <div class="bg-white p-12 rounded-[2rem] shadow-2xl border border-slate-200">
        ...
        <p class="text-slate-600 text-lg mb-8">Terima kasih, pembayaran Anda telah berhasil. Berikut detail tiket Anda.</p>
        <div class="bg-slate-50 rounded-3xl p-8 text-left shadow-sm border border-slate-200">
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-xl font-bold">{{ $transaction->event->title }}</p>
                </div>
                <span class="px-4 py-2 rounded-full bg-emerald-100 text-emerald-700 font-bold">{{ $transaction->status }}</span>
            </div>
            <div class="grid grid-cols-1 gap-4">
                ...
                <div class="flex justify-between text-slate-600">
                    <span>Tanggal</span>
                    <span>{{ \Carbon\Carbon::parse($transaction->event->date)->format('d F Y') }}</span>
                </div>
                <div class="flex justify-between text-slate-600">
                    <span>Lokasi</span>
